...
class AtomEntry(xmlmodels.XmlModel):
    ...
    def escape_xhtml(self, context, nodes):
        return "".join([etree.tounicode(strip_namespaces(n)) for n in nodes])
...
class AtomFeed(xmlmodels.XmlModel):
    ...
    updated = xmlmodels.XPathDateTimeField(
        "/atom:feed/atom:*[%s]"
        % "local-name()='updated' or (local-name()='published' and not(../atom:updated))"
    )

    entries = xmlmodels.EmbeddedXPathListField(AtomEntry, "/atom:feed/atom:entry", required=False)
